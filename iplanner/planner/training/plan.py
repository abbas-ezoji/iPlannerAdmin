...
NAME = 'iPlannerWebApi_new' # settings.DATABASES['default']['NAME']
...
for city in cities:
    ...
    const_df = pd.read_sql_query('SELECT * FROM Attraction WHERE AttractionType>0',
                                 con=engine)
    
    vst_time_from = np.array(const_df['VisitTimeFrom'])
    vst_time_to = np.array(const_df['VisitTimeTo'])
    points = np.array(const_df['Id'])
    rq_time = np.array(const_df['VisitDuration'])
    types = np.array(const_df['AttractionType'])
    len_points = len(points)
    ...
    df_city_total = df_city_total.sort_values(by='iPlannerRate', ascending=False)
    
    if len(df_city_total)<=6:
        continue
    
    for days in range(1,7):
        print('city: ' + str(city) + ' - days: ' + str(days))
        for i in range(5):
            coh_pnlty = 10
            ...
            for day in range(1,days+1):
                ...
                rq_time_mean = np.sum(df_city['VisitDuration']*df_city['rate']) / np.sum(df_city['rate'])
                ...
                vst_time_from = np.array(df_city['VisitTimeFrom'])
                vst_time_to = np.array(df_city['VisitTimeTo'])
                points = np.array(df_city['Id'])
                rq_time = np.array(df_city['VisitDuration'])
                types = np.array(df_city['AttractionType'])
